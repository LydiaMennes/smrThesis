...
class GridElem:
	...
	def check_neighbor(self, n):
		if self.id == n[1]:
			"compared to self"
		if len(self.dists) < max_closest or n[0] < self.dists[-1]:
...
def stats_to_file(iter, trial, to_follow, nr_inits, grid_size, png_nr):
	...
	plt.close()
	# grid to file 
	grid_to_file(grid_size, "stats_init" +str(nr_inits)+  "_tr" +str(trial)+"_it"+ str(iter))
	...
	if data_case_name=="test3":
		plt.pcolor(xp, yp, stress_values, cmap="PuRd")
	else:
