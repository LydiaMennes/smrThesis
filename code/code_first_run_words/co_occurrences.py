...
query = "SELECT itemText, pubDate FROM mergednewsitems WHERE sourceType = "
...
def get_connection_old():
    c = oursql.connect(host="10.0.0.125", # your host, usually localhost
...
def get_connection(d):
    if d == "before2011":
        return get_connection_beforerjan2011()
    elif d == "after2011":
        return get_connection_afterjan2011()
    else:
...
def stats_to_file(doc_nr, total_nr_words, nr_words, nr_entries, nr_words_single_freq, nr_included_words, use_stemmer, mindate,maxdate):
    ...
    f.write("minimum encountered date " + str(mindate) + "\n")
    f.write("maximum encountered date " + str(maxdate) + "\n")
    f.close()    
...
def get_cooccurrences(folder, use_stemmer):
    ...
    maxdate = datetime.date(2007,1,1)
    
    data_bases = ["after2011", "before2011"]
    
    for d in data_bases:
        
        print("Database", d)
        
        if max_nr_docs==doc_nr:
            break
        ...
        if d == "before2011":
            curquery = query.replace("mergednewsitems","newsitems")
        elif d == "after2011":
            curquery = query
        ...
        for row in curs:
            ...
            current_word = -1;
            s = row['itemText']
            # print(type(row['pubDate']))
            # print(type(row['pubDate'].date()))
            
            if row['pubDate'] != None:                
                curdate = row['pubDate'].date()
                if curdate > maxdate:
                    maxdate = row['pubDate'].date()
                if curdate <mindate:
                    mindate = row['pubDate'].date()
            ...
            if doc_nr%20000 == 0 and doc_nr!=0:
                print( doc_nr, "files, max files", max_nr_docs ,"processed at", datetime.datetime.now())
            doc_nr +=1   
            if max_nr_docs==doc_nr:
                break
    # Counts and list of words to file
    
    print("number of documents: " , doc_nr)
    ...
    stats_to_file(doc_nr, total_nr_words, nr_words, nr_entries, nr_words_single_freq, nr_included_words, use_stemmer, mindate, maxdate)    
    
    cooc_stats_to_file(cooc, tf_idf)    
    
    print("min date", mindate, "maxdate", maxdate)
    del cooc
...
if __name__ == "__main__":
    ...
    parser.add_argument("--max_nr_docs", type = int, default=None)
    ...
    use_stemmer = False
    if kwargs["use_stemmer"]=="yes":
        use_stemmer = True
    
    
    max_nr_docs =  kwargs["max_nr_docs"]
