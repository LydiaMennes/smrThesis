...
class PCA_ext:
	...
	def pca_large(self, n, k):
		...
		# G = np.array([[-0.44, 1.67, -1.26, -2.43],[0.56, 0.63,1.16,0.68],[0.69,0.09,0.45,0.65],[-0.14,-1.51,0.81,1.81],[-0.47,0.19,-0.51,-0.84]])
		# print "\nG:\n", G
		...
		while self.processed != n:
			...
			# print "size portion", p.shape
			# print "size portion H(0)", np.dot(p, G).shape
			H[ self.processed:self.processed+s, :l ] = np.dot(p, G)
			del p			
			self.processed += s
		self.new_round_data()
		del G
		# print "\nH0\n", H[ self.processed:self.processed+s, :l ]
		...
		for iter in range(1,i+1):
			...
			while self.processed != n:
				...
				# print "H_t shape", H_t.shape, "processed", self.processed, s
				# print "result shape", np.dot(p, H_t).shape
				# print "matrix multiplication", np.dot(p, H_t)
		...
		while self.processed != n:
			...
			for row in range(d):
				for column in range(q):
					# print "row", row, "column", column, "s", s, "p", p.shape
					# print "added value T", row, column, np.dot( p[:,row] , Q[self.processed:self.processed+s , column])
		...
		"step 5"
		U = np.dot(Q,W.T)
		# print "U", U.shape, "S", np.diag(S).shape, "V", V.shape
		# print "reconstruction:\n", np.dot( np.dot(U[:n, :k], np.diag(S)[:k, :k] ), V[:d, :k].T)
		del Q
		del W
	...
	def get_portion_A(self):
		...
		# print "data retrieval iteration size", s
