...
def Hbeta(D = Math.array([]), beta = 1.0):
	...
	micro_value = 0.000000000001
	P = Math.exp(-D.copy() * beta)	
	# P = Math.exp(-D.copy())	
	
	sumP = sum(P);
	# Hier zit waarschijnlijk toch het probleem: delen door 0 met sumP
	# Wat wil je in dat geval?????????
	H = 0
	P2 = 0
	if sumP == 0:
		H = Math.log(sumP) + beta * Math.sum(D * P) / micro_value;
		P2 = P / micro_value
	else:
...
def pca_cov(X = Math.array([]), no_dims = 50):
	"""Runs PCA on the NxD array X in order to reduce its dimensionality to no_dims dimensions."""
	X -= Math.mean(X, axis=0) # subtract mean
	[latent, coeff] = Math.linalg.eig(Math.cov(X.T)) # use covariance matrix
	idx = Math.argsort(latent) # sorting eigenvalues
	idx = idx[::-1]	# in ascending order
	coeff = coeff[:,idx]
	coeff = coeff[:,range(no_dims)]
	Y = Math.dot(coeff.T, X.T)
	return Y.T


def tsne(X = Math.array([]), no_dims = 2, initial_reduction = 50, perplexity = 30.0, already_reduced=False):
...
def data_to_file(data, label):
	f = open("minst_data_reduced.txt", 'w')
	for i in range(data.shape[0]):
		f.write(str(data[i,0]) + " " + str(data[i,1])+" "+str(int(label[i]))+"\n")
	f.close()
